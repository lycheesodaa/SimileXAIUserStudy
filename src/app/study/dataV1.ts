...
// ─── Example bank (examples/prototype condition cheatsheet) ──────────────────
// Per-class representative training recordings, precomputed from the proto
// model's prototype bank across the bundle's samples (the most frequently
// selected prototypes per class). Committed at <root>/<domain>/example_bank.json
// so the examples-condition cheatsheet can show a few real training examples per
// category without loading every sample at runtime. Regenerated by
// dev/build_example_bank.py. Missing file resolves undefined (older bundles).

export interface ExampleBankClip {
  /** Source training-clip filename (opaque; no class label). */
  source: string;
  audio: string;
}

export interface ExampleBankEntry {
  label: string;
  clips: ExampleBankClip[];
}

export function loadExampleBank(
  domain: string,
  root: DataRoot = DATA_V1_ROOT
